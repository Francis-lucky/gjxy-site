...
export default function CooperationPage() {
  ...
  const totalPages = Math.ceil(partnerUniversitiesData.length / itemsPerPage)
  const startIndex = (currentPage - 1) * itemsPerPage
  const endIndex = startIndex + itemsPerPage
  const currentItems = partnerUniversitiesData.slice(startIndex, endIndex)
      ...
      <div className="bg-white border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-4 py-3">
          <div className="flex items-center gap-2 text-sm text-gray-600">
            ...
            <span>{">"}</span>
            <span className="text-gray-800">合作院校</span>
          </div>
        </div>
      </div>
              ...
              <div className="px-6 py-4 border-t border-gray-200 bg-gray-50">
                <div className="flex items-center justify-between">
                  <div className="text-sm text-gray-600">
                    展示第{startIndex + 1}-{Math.min(endIndex, partnerUniversitiesData.length)}条信息，共计
                    {partnerUniversitiesData.length}
                    条信息
                  </div>
