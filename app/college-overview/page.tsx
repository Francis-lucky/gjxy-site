...
export default function CollegeOverviewPage() {
  ...
  const [activeSection, setActiveSection] = useState("学院简介")
  ...
  const partnerUniversitiesData = [
    { title: "与俄罗斯远东联邦大学签署全面战略合作协议", date: "2025.08.20" },
    { title: "莫斯科国立大学代表团访问我院并达成合作意向", date: "2025.08.15" },
    { title: "圣彼得堡国立大学建立学生交换项目", date: "2025.08.10" },
  ]

  const leadershipData = [{ title: "学院领导班子调整公告 - 新任院长张教授正式就职", date: "2025.08.20" }]

  useEffect(() => {
    const section = searchParams.get("section")
    if (section === "cooperation") {
      setActiveSection("合作院校")
    } else if (section === "leadership") {
      setActiveSection("领导分工")
    } else {
      setActiveSection("学院简介")
    }
  }, [searchParams])
  ...
  const getCurrentData = () => {
    switch (activeSection) {
      case "学院简介":
        return collegeIntroData
      case "合作院校":
        return partnerUniversitiesData
      case "领导分工":
      ...
      <div className="bg-white border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-4 py-3">
          <div className="flex items-center gap-2 text-sm text-gray-600">
            ...
            <span>{">"}</span>
            <span className="text-gray-800">{activeSection}</span>
          </div>
        </div>
      </div>
              ...
              <div className="px-6 py-4 border-t border-gray-200 bg-gray-50">
                <div className="flex items-center justify-between">
                  <div className="text-sm text-gray-600">
                    展示第{startIndex + 1}-{Math.min(endIndex, currentData.length)}条信息，共计{currentData.length}
                    条信息
                  </div>
