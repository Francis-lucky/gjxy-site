...
export default function CollegeIntroductionPage() {
  ...
  const collegeIntroData = [
    { title: "《重庆邮电大学全面落实研究生指导教师立德树人职责实施细则》", date: "2025.08.20" },
    { title: "重庆邮电大学远东联邦大学联合学院成立暨揭牌仪式隆重举行", date: "2025.08.15" },
    { title: "学院与俄罗斯远东联邦大学签署合作办学协议", date: "2025.08.10" },
    { title: "国际化人才培养模式创新与实践研讨会成功举办", date: "2025.08.05" },
    { title: "学院首届中俄联合培养学生顺利毕业", date: "2025.07.30" },
    { title: "中俄教育合作项目获得教育部正式批准", date: "2025.07.25" },
    { title: "学院教学设施建设项目全面完工", date: "2025.07.20" },
    { title: "国际学院师资队伍建设取得重要进展", date: "2025.07.15" },
    { title: "学院与多家知名企业建立实习合作关系", date: "2025.07.10" },
    { title: "首批俄语教学课程正式开设", date: "2025.07.05" },
    { title: "学院学生在国际竞赛中获得优异成绩", date: "2025.06.30" },
    { title: "中俄文化交流活动周圆满结束", date: "2025.06.25" },
    { title: "学院正式获得中外合作办学许可证", date: "2025.06.20" },
  ]

  const totalPages = Math.ceil(collegeIntroData.length / itemsPerPage)
  const startIndex = (currentPage - 1) * itemsPerPage
  const endIndex = startIndex + itemsPerPage
  const currentItems = collegeIntroData.slice(startIndex, endIndex)
  ...
  const handleSideNavClick = (section: string) => {
    if (section === "学院简介") {
      router.push("/college-overview/introduction")
    } else if (section === "合作院校") {
      router.push("/college-overview/cooperation")
    } else if (section === "领导分工") {
      router.push("/college-overview/leadership")
    }
  }
  ...
  const handleArticleClick = (item: any, index: number) => {
    // Navigate to detail page for the first article
    if (index === 0) {
      router.push("/college-overview/introduction/detail")
    } else {
      ...
      <div className="bg-white border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-4 py-3">
          <div className="flex items-center gap-2 text-sm text-gray-600">
            ...
            <span>{">"}</span>
            <span className="text-blue-500">学院概况</span>
            <span>{">"}</span>
            <span className="text-gray-800">学院简介</span>
          </div>
        </div>
      </div>
          ...
          <aside className="w-64 flex-shrink-0">
            <div className="bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden">
              <div className="bg-teal-500 text-white px-4 py-3">
                <h2 className="text-lg font-semibold">学院概况</h2>
              </div>
              <nav className="py-2">
                {["学院简介", "合作院校", "领导分工"].map((item) => (
                  <button
                    key={item}
                  ...
                  {currentItems.map((item, index) => (
                    <li key={index} className="flex items-start gap-4 py-3 border-b border-gray-100 last:border-b-0">
                      ...
                      <div className="flex-1">
                        <a
                          href="#"
                          className="text-gray-700 hover:text-teal-600 transition-colors leading-relaxed cursor-pointer"
                          onClick={(e) => {
                            e.preventDefault()
              ...
              <div className="px-6 py-4 border-t border-gray-200 bg-gray-50">
                <div className="flex items-center justify-between">
                  <div className="text-sm text-gray-600">
                    展示第{startIndex + 1}-{Math.min(endIndex, collegeIntroData.length)}条信息，共计
                    {collegeIntroData.length}
                    条信息
                  </div>
