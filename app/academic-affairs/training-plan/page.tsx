...
export default function TrainingPlanPage() {
  ...
  const trainingPlanData = [
    { title: "（测试）计算机科学与技术专业培养方案（2025版）", date: "2025.08.20" },
    { title: "（测试）电子信息工程专业培养方案（2025版）", date: "2025.08.18" },
    { title: "（测试）自动化专业培养方案（2025版）", date: "2025.08.15" },
    { title: "（测试）专业课程体系建设指导意见", date: "2025.08.10" },
    { title: "（测试）实践教学环节实施方案", date: "2025.08.05" },
    { title: "（测试）双语教学课程设置标准", date: "2025.07.30" },
    { title: "（测试）毕业设计（论文）管理办法", date: "2025.07.25" },
    { title: "（测试）创新创业教育实施细则", date: "2025.07.20" },
    { title: "（测试）国际交流项目学分认定办法", date: "2025.07.15" },
    { title: "（测试）专业实习实训管理规定", date: "2025.07.10" },
    { title: "（测试）课程考核评价体系", date: "2025.07.05" },
    { title: "（测试）学分制管理实施办法", date: "2025.06.30" },
  ]

  const totalPages = Math.ceil(trainingPlanData.length / itemsPerPage)
  const startIndex = (currentPage - 1) * itemsPerPage
  const endIndex = startIndex + itemsPerPage
  const currentItems = trainingPlanData.slice(startIndex, endIndex)
  ...
  const handleSideNavClick = (section: string) => {
    if (section === "培养模式") {
      router.push("/academic-affairs/training-mode")
    } else if (section === "培养方案") {
      router.push("/academic-affairs/training-plan")
    } else if (section === "教务管理") {
      router.push("/academic-affairs/management")
    }
  }
      ...
      <div className="bg-white border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-4 py-3">
          <div className="flex items-center gap-2 text-sm text-gray-600">
            ...
            <span>{">"}</span>
            <span className="text-blue-500">教学教务</span>
            <span>{">"}</span>
            <span className="text-gray-800">培养方案</span>
          </div>
        </div>
      </div>
          ...
          <div className="w-64 flex-shrink-0">
            <div className="bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden">
              <div className="bg-teal-500 text-white px-4 py-3">
                <h2 className="text-lg font-semibold">教学教务</h2>
              </div>
              <nav className="py-2">
                {["培养模式", "培养方案", "教务管理"].map((item) => (
                  <button
                    key={item}
              ...
              <div className="px-6 py-4 border-t border-gray-200 bg-gray-50">
                <div className="flex items-center justify-between">
                  <div className="text-sm text-gray-600">
                    展示第{startIndex + 1}-{Math.min(endIndex, trainingPlanData.length)}条信息，共计
                    {trainingPlanData.length}
                    条信息
                  </div>
