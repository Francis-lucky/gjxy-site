...
export default function ManagementPage() {
  ...
  const managementData = [
    { title: "（测试）教学管理制度汇编（2025版）", date: "2025.08.20" },
    { title: "（测试）学籍管理实施细则", date: "2025.08.18" },
    { title: "（测试）课程考试管理办法", date: "2025.08.15" },
    { title: "（测试）教学质量监控体系建设方案", date: "2025.08.12" },
    { title: "（测试）学生成绩管理规定", date: "2025.08.10" },
    { title: "（测试）教务管理系统使用指南", date: "2025.08.08" },
    { title: "（测试）选课管理办法", date: "2025.08.05" },
    { title: "（测试）教师教学工作规范", date: "2025.08.03" },
    { title: "（测试）学位授予工作实施细则", date: "2025.08.01" },
    { title: "（测试）转专业管理办法", date: "2025.07.30" },
    { title: "（测试）休学复学管理规定", date: "2025.07.28" },
    { title: "（测试）教学督导工作实施方案", date: "2025.07.25" },
    { title: "（测试）学生评教管理办法", date: "2025.07.22" },
    { title: "（测试）教学事故认定与处理办法", date: "2025.07.20" },
  ]

  const totalPages = Math.ceil(managementData.length / itemsPerPage)
  const startIndex = (currentPage - 1) * itemsPerPage
  const endIndex = startIndex + itemsPerPage
  const currentItems = managementData.slice(startIndex, endIndex)
      ...
      <div className="bg-white border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-4 py-3">
          <div className="flex items-center gap-2 text-sm text-gray-600">
            ...
            <span>{">"}</span>
            <span className="text-gray-800">教务管理</span>
          </div>
        </div>
      </div>
              ...
              <div className="px-6 py-4 border-t border-gray-200 bg-gray-50">
                <div className="flex items-center justify-between">
                  <div className="text-sm text-gray-600">
                    展示第{startIndex + 1}-{Math.min(endIndex, managementData.length)}条信息，共计
