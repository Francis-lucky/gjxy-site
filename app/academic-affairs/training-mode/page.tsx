...
export default function TrainingModePage() {
  ...
  const trainingModeData = [
    { title: "（测试）国际化人才培养模式实施方案", date: "2025.08.20" },
    { title: "（测试）中俄联合培养4+0模式介绍", date: "2025.08.15" },
    { title: "（测试）双语教学实施细则", date: "2025.08.10" },
    { title: "（测试）国际师资队伍建设规划", date: "2025.08.05" },
    { title: "（测试）跨文化交流能力培养方案", date: "2025.07.30" },
    { title: "（测试）双学位认证管理办法", date: "2025.07.25" },
    { title: "（测试）专业课程设置标准", date: "2025.07.20" },
    { title: "（测试）俄语教学大纲", date: "2025.07.15" },
    { title: "（测试）实践教学环节安排", date: "2025.07.10" },
    { title: "（测试）学生交流项目管理规定", date: "2025.07.05" },
    { title: "（测试）教学质量评估体系", date: "2025.06.30" },
    { title: "（测试）国际化课程建设指南", date: "2025.06.25" },
  ]

  const totalPages = Math.ceil(trainingModeData.length / itemsPerPage)
  const startIndex = (currentPage - 1) * itemsPerPage
  const endIndex = startIndex + itemsPerPage
  const currentItems = trainingModeData.slice(startIndex, endIndex)
      ...
      <div className="bg-white border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-4 py-3">
          <div className="flex items-center gap-2 text-sm text-gray-600">
            ...
            <span>{">"}</span>
            <span className="text-gray-800">培养模式</span>
          </div>
        </div>
      </div>
              ...
              <div className="px-6 py-4 border-t border-gray-200 bg-gray-50">
                <div className="flex items-center justify-between">
                  <div className="text-sm text-gray-600">
                    展示第{startIndex + 1}-{Math.min(endIndex, trainingModeData.length)}条信息，共计
                    {trainingModeData.length}
                    条信息
                  </div>
